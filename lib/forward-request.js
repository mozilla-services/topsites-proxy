...
const REQUEST_PROPS_TOCOPY = ["host", "hostname", "socketPath", "pfx", "key",
  "passphrase", "cert", "ca", "ciphers", "secureProtocol"];
const HEADER_UPGRADE_RE = /(^|,)\s*upgrade\s*($|,)/i;

const PROTO_PORT_MAP = new Map([
  ["http", 80], ["ws", 80],
  ["https", 443], ["wss", 443],
  ["ftp", 21],
  ["gopher", 70],
...
function isPortRequired(port, protocol) {
  ...
  if (PROTO_PORT_MAP.has(protocol)) {
    return PROTO_PORT_MAP.get(protocol);
  }
...
function getRequestOptions(req, url, options = {}) {
  ...
  options.headers.host = isPortRequired(options.port, url.protocol) && !hasPort(options.host)
  ...
  if (typeof options.headers.connection != "string" || !HEADER_UPGRADE_RE.test(options.headers.connection)) {
...
module.exports = function forwardRequest(req, res, options) {
  let url = parse_url(options.target);
  let isSecure = (url.protocol == "https:");
  
  let forwardReq = (isSecure ? https : http).request(getRequestOptions(req, url));
  ...
  let proxyError = createErrorHandler(req, forwardReq, options.target);
