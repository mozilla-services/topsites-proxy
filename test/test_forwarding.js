...
describe("Top Sites forward request endpoint", function() {
  ...
  it("should handle proper requests to /cid/:cid properly", async function() {
    return withServer(async server => {
      ...
      Assert.ok(data);
      Assert.equal(data.trim(), `TEST: /test?sub1=amazon&key=xxx&cuid=${cid}&h1=&h2=&cu=`);
    });
  });
  ...
  it("should handle proper requests to /cid/:cid properly WITH headers", async function() {
    return withServer(async server => {
      ...
      Assert.ok(query);
      Assert.equal(query.trim(), `TEST: /test?sub1=amazon&key=xxx&cuid=${cid}&h1=us&h2=newtab&cu=`);
  ...
  it("should handle proper requests to /cid/:cid replacing gb with uk", async function() {
    return withServer(async server => {
      ...
      Assert.ok(data);
      Assert.equal(data.trim(), `TEST: /test?sub1=amazon&key=xxx&cuid=${cid}&h1=uk&h2=newtab&cu=`);
    });
  });
  ...
  it("should send to correct data for eBay campaigns", async function() {
    return withServer(async server => {
      const cid = "amzn_2020_1";
      const targetURL = "https://www.ebay.co.uk?mkevt=1&mkcid=2&mkrid=" +
